...
const MigrantSearchPage: React.FC<MigrantSearchPageProps> = ({ onSelectMigrant }) => {
  ...
  const [results, setResults] = useState<MigrantProfile[]>([]);
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    setSearched(true);
    if (searchTerm.trim() === '') {
      setResults(dummyMigrants); // Show all if search is empty
    } else {
      const lowercasedTerm = searchTerm.toLowerCase();
      setResults(
        dummyMigrants.filter(
          m =>
      ...
      <Card>
        ...
        {searched && results.length === 0 ? (
          ...
          <ul className="space-y-4">
            {(searched ? results : dummyMigrants).map((migrant, index) => (
