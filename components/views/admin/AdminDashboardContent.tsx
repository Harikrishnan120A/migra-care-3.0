...
const StatCard: React.FC<{ title: string; value: string; icon: JSX.Element; colorClass: string; delay: string }> = ({ title, value, icon, colorClass, delay }) => (
    ...
    return (
        <div className="space-y-8">
             <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
                <StatCard title="Total Migrants" value="1,450" icon={<UsersIcon />} colorClass="text-accent-cyan" delay="0.1s" />
                <StatCard title="Total Doctors" value="210" icon={<StethoscopeIcon />} colorClass="text-accent-cyan" delay="0.2s" />
                <StatCard title="Appointments" value="893" icon={<CalendarIcon />} colorClass="text-text-primary" delay="0.3s" />
