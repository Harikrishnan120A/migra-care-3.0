...
const migrantLinks: NavLink[] = [
  ...
  { label: 'Health Records', icon: <HealthRecordIcon /> },
  { label: 'Appointments', icon: <AppointmentIcon /> },
  { label: 'AI Chatbot', icon: <ChatIcon /> },
];

const doctorLinks: NavLink[] = [
  { label: 'Dashboard', icon: <DashboardIcon /> },
  { label: 'Migrant Search', icon: <SearchIcon /> },
];
...
const Sidebar: React.FC<SidebarProps> = ({ activeView = 'Dashboard', onNavigate }) => {
  ...
  return (
    <aside className="flex flex-col w-full h-full p-4 overflow-hidden bg-glass-bg/70 backdrop-blur-lg border-r border-glow-border transition-all duration-300">
      <div className="flex items-center mb-10 flex-shrink-0">
        <AppLogoIcon />
        <h2 className={`ml-3 text-2xl font-bold text-white transition-opacity duration-200 whitespace-nowrap ${isSidebarOpen ? 'opacity-100' : 'opacity-0'}`}>KMHR</h2>
      </div>
          ...
          {links.map((link) => (
            <li key={link.label} className="relative group">
              <button
                ...
                className={`flex items-center w-full p-3 my-1 text-left transition-all duration-300 transform rounded-md ${activeView === link.label ? 'bg-accent-cyan/20 text-accent-cyan font-bold shadow-lg' : 'text-text-secondary hover:bg-white/10 hover:text-white'} ${!isSidebarOpen && 'justify-center'}`}
              ...
              >
                ...
                <span className={`mx-4 font-medium transition-opacity duration-200 whitespace-nowrap ${isSidebarOpen ? 'opacity-100' : 'opacity-0'}`}>{link.label}</span>
              </button>
              {!isSidebarOpen && (
