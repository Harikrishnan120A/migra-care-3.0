...
const AppointmentsPage: React.FC = () => {
    const upcomingAppointments = dummyAppointments.filter(a => a.status === 'Upcoming');
    const pastAppointments = dummyAppointments.filter(a => a.status !== 'Upcoming').sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    ...
    return (
        <div className="space-y-8">
            ...
            <Card className="slide-in-up-content" style={{animationDelay: '100ms'}}>
                <h3 className="mb-4 text-xl font-semibold text-white">Upcoming</h3>
                {upcomingAppointments.length > 0 ? (
                    ...
                    <p className="px-4 py-6 text-center bg-black/20 rounded-md text-text-secondary">No upcoming appointments.</p>
                )}
            </Card>

            <Card className="slide-in-up-content" style={{animationDelay: '300ms'}}>
                <h3 className="mb-4 text-xl font-semibold text-white">History</h3>
                {pastAppointments.length > 0 ? (
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                        {pastAppointments.map((app, index) => <AppointmentCard key={app.id} appointment={app} style={{animationDelay: `${index * 100 + 400}ms`}} />)}
                    </div>
                ) : (
                     <p className="px-4 py-6 text-center bg-black/20 rounded-md text-text-secondary">No past appointments.</p>
                )}
            </Card>
        </div>
    );
